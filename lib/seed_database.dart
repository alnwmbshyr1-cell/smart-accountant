...
class SeedDatabase {
  ...
  static const List<String> _namesList = [
    "أحمد", "محمد", "خالد", "علي", "سالم", "فهد", "عبدالله", "عمر", "إبراهيم", "يوسف",
    "ناصر", "صالح", "حسن", "حسين", "معاذ", "بلال", "عارف", "سامي", "نبيل", "منصور",
    "جمال", "رائد", "زياد", "باسل", "هيثم", "طارق", "واصف", "أمين", "محسن", "عادل"
  ];

  static const List<String> _notesList = [
    "بضاعة", "بنزين", "إيجار", "رواتب", "مواد غذائية", "قطع غيار", "صيانة", "كهرباء ومياه", "نقل وتوصيل", "أمشاج"
  ];

  static const List<String> _typesList = [
    "مبيعات", "مشتريات", "مصروف", "دين لك", "دين عليك"
  ];
  ...
  static Future<void> generateMassiveData({
    ...
    for (int batchStart = 0; batchStart < totalRecords; batchStart += batchSize) {
      int currentBatchCount = (batchStart + batchSize > totalRecords) ? (totalRecords - batchStart) : batchSize;
      ...
      for (int i = 0; i < currentBatchCount; i++) {
        ...
        double amount = (random.nextInt(10000) + 1) * 1000.0; // مبالغ من 1000 إلى 10,000,000
        ...
        String dateStr = DateTime.now().subtract(Duration(daysdays: daysAgo)).toIso8601String();
      ...
      onProgress(createdCount, totalRecords, "جاري حقن الدفعة: $createdCount من $totalRecords سجل...");
