...
class _HomeShellState extends State<HomeShell> {
  ...
  @override Widget build(BuildContext context) {
    final pages = [HomeScreen(db: widget.db, refresh: refresh, onAdd: () => setState(() => index = 3), onHealth: () => Navigator.push(context, MaterialPageRoute(builder: (_) => HealthRecordScreen(db: widget.db))), onReports: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ReportsScreen(db: widget.db)))), VaccinationScreen(db: widget.db), TagsScreen(db: widget.db, refresh: refresh), AddAnimalScreen(db: widget.db, onSaved: () { refresh.value++; setState(() => index = 0); })];
...
class _HealthRecordScreenState extends State<HealthRecordScreen> { List<Map<String, dynamic>> records = []; String filter = 'الكل'; @override void initState() { super.initState(); load(); } Future<void> load() async { final rows = await widget.db.healthRecords(); if (mounted) setState(() => records = rows); } Future<void> addRecord() async { final animal = TextEditingController(); final condition = TextEditingController(); final notes = TextEditingController(); String status = 'قيد المتابعة'; final saved = await showDialog<bool>(context: context, builder: (_) => StatefulBuilder(builder: (context, setDialog) => AlertDialog(title: const Text('إضافة حالة صحية'), content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [TextField(controller: animal, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'رقم الرأس')), const SizedBox(height: 10), TextField(controller: condition, decoration: const InputDecoration(labelText: 'الحالة / التشخيص')), const SizedBox(height: 10), DropdownButtonFormField<String>(value: status, decoration: const InputDecoration(labelText: 'الحالة'), items: ['قيد المتابعة', 'تماثل للشفاء', 'مغلقة'].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (v) => setDialog(() => status = v!)), const SizedBox(height: 10), TextField(controller: notes, maxLines: 2, decoration: const InputDecoration(labelText: 'ملاحظات'))])), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('إلغاء')), FilledButton(onPressed: () async { if (animal.text.trim().isNotEmpty && condition.text.trim().isNotEmpty) { await widget.db.addHealthRecord(animal.text.trim(), condition.text.trim(), notes.text.trim(), status); Navigator.pop(context, true); } }, child: const Text('حفظ'))])); if (saved == true) load(); }
...
class LivestockDb {
  ...
  Future<void> open() async {
    final path = p.join(await getDatabasesPath(), 'maqani.db');
    _db = await openDatabase(path, version: 2, onCreate: (db, _) async {
      await _createTables(db);
      final batch = db.batch();
      final seed = [
        {'number': '101', 'tag_color': 'برتقالي', 'animal_type': 'نجدي', 'gender': 'انثى'},
        {'number': '102', 'tag_color': 'أخضر', 'animal_type': 'حري', 'gender': 'ذكر'},
        {'number': '103', 'tag_color': 'أزرق', 'animal_type': 'معز', 'gender': 'انثى'},
        {'number': '104', 'tag_color': 'برتقالي', 'animal_type': 'نجدي', 'gender': 'ذكر'},
      ];
      for (final a in seed) batch.insert('animals', a);
      batch.insert('financial_entries', {'kind': 'income', 'amount': 4200.0, 'category': 'بيع رؤوس', 'note': 'مبيعات الشهر'});
      batch.insert('financial_entries', {'kind': 'expense', 'amount': 1250.0, 'category': 'أعلاف', 'note': 'شراء أعلاف'});
      await batch.commit();
  ...
  static Future<void> _createTables(Database db) async {
    await db.execute('CREATE TABLE IF NOT EXISTS animals (id INTEGER PRIMARY KEY AUTOINCREMENT, number TEXT NOT NULL, tag_color TEXT NOT NULL, animal_type TEXT NOT NULL, gender TEXT NOT NULL, birth_date TEXT)');
    await db.execute('CREATE TABLE IF NOT EXISTS tags (id INTEGER PRIMARY KEY AUTOINCREMENT, number TEXT NOT NULL, color TEXT NOT NULL)');
    await db.execute('CREATE TABLE IF NOT EXISTS health_records (id INTEGER PRIMARY KEY AUTOINCREMENT, animal_number TEXT NOT NULL, condition TEXT NOT NULL, notes TEXT, status TEXT NOT NULL, date TEXT NOT NULL)');
    await db.execute('CREATE TABLE IF NOT EXISTS financial_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, kind TEXT NOT NULL, amount REAL NOT NULL, category TEXT NOT NULL, note TEXT, date TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)');
  }
  Future<List<Map<String, dynamic>>> animals() async => _db!.query('animals', orderBy: 'id DESC');
  Future<void> addAnimal(Map<String, dynamic> a) async => _db!.insert('animals', a);
  Future<List<Map<String, dynamic>>> tags() async => _db!.query('tags', orderBy: 'id DESC');
  Future<void> addTag(String n, String c) async => _db!.insert('tags', {'number': n, 'color': c});
  Future<void> deleteTag(int id) async => _db!.delete('tags', where: 'id = ?', whereArgs: [id]);
  Future<List<Map<String, dynamic>>> healthRecords() async => _db!.query('health_records', orderBy: 'id DESC');
  Future<void> addHealthRecord(String animal, String condition, String notes, String status) async => _db!.insert('health_records', {'animal_number': animal, 'condition': condition, 'notes': notes, 'status': status, 'date': DateTime.now().toIso8601String()});
  Future<List<Map<String, dynamic>>> financialEntries() async => _db!.query('financial_entries', orderBy: 'id DESC');
  Future<void> addFinancialEntry(String kind, double amount, String category, String note) async => _db!.insert('financial_entries', {'kind': kind, 'amount': amount, 'category': category, 'note': note});
}
